...
function App() {
  return (
    <main className="min-h-screen overflow-hidden bg-[var(--paper)] text-stone-900">
      ...
      <section className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 pb-16 pt-8 md:px-10 lg:px-12">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
          <div className="space-y-6">
            <p className="inline-flex rounded-full border border-stone-900/10 bg-white/70 px-4 py-2 text-xs font-semibold uppercase tracking-[0.3em] text-stone-700 shadow-sm backdrop-blur">
              Returning teammate, familiar face
            </p>

            <div className="space-y-4">
              <h1 className="max-w-4xl text-5xl font-semibold leading-none tracking-[-0.04em] text-balance sm:text-6xl lg:text-7xl">
            ...
            <ul className="grid gap-3 pt-4 text-sm text-stone-700 sm:grid-cols-3">
              {highlights.map((item) => (
                <li
                  key={item}
                  className="rounded-3xl border border-stone-900/10 bg-white/70 p-4 shadow-[0_10px_30px_rgba(28,25,23,0.06)] backdrop-blur"
                >
                  {item}
                </li>
              ))}
          ...
          <aside className="relative">
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-stone-900/10 bg-stone-950 p-6 text-stone-50 shadow-[0_24px_80px_rgba(28,25,23,0.18)]">
              <div className="space-y-6">
      ...
      <section id="story" className="mx-auto w-full max-w-6xl px-6 py-10 md:px-10 lg:px-12">
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          {timeline.map((item, index) => (
            <article
              key={item.team}
              className="relative overflow-hidden rounded-[2rem] border border-stone-900/10 bg-white p-6 shadow-[0_18px_60px_rgba(28,25,23,0.08)]"
            >
      ...
      <section className="mx-auto w-full max-w-6xl px-6 py-10 md:px-10 lg:px-12">
        <div className="grid gap-6 rounded-[2rem] border border-stone-900/10 bg-stone-950 px-6 py-8 text-stone-50 shadow-[0_28px_80px_rgba(28,25,23,0.14)] lg:grid-cols-[0.9fr_1.1fr] lg:px-8">
          <div>
      ...
      <section className="mx-auto w-full max-w-6xl px-6 py-10 md:px-10 lg:px-12">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-5">
          {personalBits.map((item) => (
            <article
              key={item.label}
              className="rounded-[2rem] border border-stone-900/10 bg-white/85 p-6 shadow-[0_18px_50px_rgba(28,25,23,0.07)] backdrop-blur"
            >
      ...
      <section id="portrait" className="mx-auto w-full max-w-6xl px-6 py-10 md:px-10 lg:px-12">
        <div className="rounded-[2rem] border border-stone-900/10 bg-white/80 p-8 shadow-[0_18px_50px_rgba(28,25,23,0.07)] backdrop-blur">
          <p className="text-sm uppercase tracking-[0.28em] text-stone-500">
            A little more life
          </p>
